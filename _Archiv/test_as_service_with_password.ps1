# Automated Service-User Testing Script with Password Input
# Tests all three PowerShell starter scripts as the Service user
# Version: 2025-10-22 (with credential support)
...
Write-TestLog "=== Starting Automated Service-User Tests (with Password) ===" "INFO"
...
$credential = Get-Credential -UserName $ServiceUser -Message "Enter password for Service user"

if (-not $credential) {
    Write-TestLog "ERROR: No credentials provided. Exiting." "ERROR"
    exit 1
}

$password = $credential.GetNetworkCredential().Password
Write-TestLog "Credentials received for user: $ServiceUser" "SUCCESS"
...
$testScripts = @(
    ...
    @{
        ...
        TaskName = "Test_Status_Service_PWD"
        Timeout = 15
    }
)
...
foreach ($test in $testScripts) {
    ...
    # Register the task WITH PASSWORD
    try {
        Register-ScheduledTask `
            ...
            -Settings $settings `
            -RunLevel Highest `
            -Description "Temporary test task for $($test.Name) as Service user (with password)" `
...
foreach ($test in $testScripts) {
    ...
    try {
        ...
        # Get last run result
        $taskLastResult = (Get-ScheduledTaskInfo -TaskName $test.TaskName).LastTaskResult
        Write-TestLog "  Last task result code: $taskLastResult" "INFO"
        ...
        if ($taskState -eq "Ready" -and $taskLastResult -ne 0) {
            Write-TestLog "  WARNING: Task completed immediately with error code: $taskLastResult" "WARNING"
            $reasons += "Task failed with error code: $taskLastResult"
        }
        ...
        # Check 3: Emergency logs in C:\Temp
        $emergencyLogs = Get-ChildItem "C:\Temp\*_emergency_*2025-10*.log" -ErrorAction SilentlyContinue |
            Where-Object { $_.LastWriteTime -gt (Get-Date).AddMinutes(-5) }

        if ($emergencyLogs) {
            Write-TestLog "  WARNING: Emergency log found in C:\Temp" "WARNING"
            foreach ($emLog in $emergencyLogs) {
                Write-TestLog "    $($emLog.Name)" "WARNING"
                $emContent = Get-Content $emLog.FullName -Tail 5 -ErrorAction SilentlyContinue
                foreach ($line in $emContent) {
                    Write-TestLog "      $line" "WARNING"
                }
            }
            $reasons += "Emergency logs found in C:\Temp"
        }
...
foreach ($result in $testResults) {
    ...
    if ($result.LastResult) {
        Write-TestLog "  Task Exit Code: $($result.LastResult)" "INFO"
    }
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  TESTING COMPLETE" -ForegroundColor $(if ($failedCount -gt 0) { "Red" } else { "Green" })
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Passed: $passedCount / Failed: $failedCount" -ForegroundColor White
